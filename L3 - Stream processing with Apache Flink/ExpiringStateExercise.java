...
public class ExpiringStateExercise extends ExerciseBase {
	...
	public static class EnrichmentFunction extends KeyedCoProcessFunction<Long, TaxiRide, TaxiFare, Tuple2<TaxiRide, TaxiFare>> {
		Map<Long, TaxiRide> ridesDict;
		Map<Long, TaxiFare> faresDict;

		@Override
		public void open(Configuration config) throws Exception {
			ridesDict = new HashMap<Long, TaxiRide>();
			faresDict = new HashMap<Long, TaxiFare>();
		}

		@Override
		public void onTimer(long timestamp, OnTimerContext ctx, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			if (faresDict.get(ctx.getCurrentKey()) != null) {
				ctx.output(unmatchedFares, faresDict.get(ctx.getCurrentKey()));
				faresDict.remove(ctx.getCurrentKey());
			}
			if (ridesDict.get(ctx.getCurrentKey()) != null) {
				ctx.output(unmatchedRides, ridesDict.get(ctx.getCurrentKey()));
				ridesDict.remove(ctx.getCurrentKey());
			}
		}

		@Override
		public void processElement1(TaxiRide ride, Context context, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			if (faresDict.get(ride.rideId) == null) {
				ridesDict.put(ride.rideId, ride);
				context.timerService().registerEventTimeTimer((context.timestamp() / 1000) + 60);
			}
			else {
				faresDict.remove(ride.rideId);
				out.collect(new Tuple2<TaxiRide, TaxiFare>(ride, faresDict.get(ride.rideId)));
			}
		}

		@Override
		public void processElement2(TaxiFare fare, Context context, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			if (ridesDict.get(fare.rideId) == null) {
				faresDict.put(fare.rideId, fare);
				context.timerService().registerEventTimeTimer((context.timestamp() / 1000) + 60);
			}
			else {
				ridesDict.remove(fare.rideId);
				out.collect(new Tuple2<TaxiRide, TaxiFare>(ridesDict.get(fare.rideId), fare));
			}
		}
	}
}